...
class PnP_APICEM:
    """ Class for managing EasyPnP in APIC-EM controller """
    ...
    def pnp_APICEM_menu_device_upload(path):
        ...
        while True:
            try:
                ...
                if command == "1":
                    # Upload devices to APIC-EM
                    PnP_APICEM.pnp_APICEM_get_project_for_post_devices()
                elif command == "2":
                    # Upload devices to APIC-EM and ISE
...
########################################################################################################################
    ...
    def pnp_APICEM_make_configuration_excel():
        ...
        if not os.path.isfile(PnP_APICEM.devices_tab) or not os.path.isfile(PnP_APICEM.template_file):
...
########################################################################################################################
                        ...
                        if (cmd - 1 >= 0) and (cmd - 1 < len(r_json["response"])):
                            ...
                            while True:
                                ...
                                if cmd2 == "n" or cmd2 == "N":
                                    ...
                                    if not serialNO == []:
                                        # Call the method with relevant parameters to delete device from APIC-EM
                                        PnP_APICEM.pnp_APICEM_get_devices_for_delete(serialNO, False)
                                    break
                                # If you want to delete the device and reload as well
                                if cmd2 == "y" or cmd2 == "Y":
    ...
    def pnp_APICEM_get_devices_for_delete(serialNO, printResp=True):
        """" Method for deleting devices from APIC-EM """
        # Save result of this API call into r_json variable
        ...
        if not r_json["response"] == []:
            try:
                ...
                for i in r_json["response"]:
                    # If SN of device in APIC-EM matches SN of device you want to delete
                    if i.get("serialNumber") == serialNO:
                        deviceID = i["id"]
                        # Use the API call method with deviceID parameter to delete device from APIC-EM
    ...
    def pnp_APICEM_get_project_for_post_devices():
        """" Method for getting all projects for posting devices to APIC-EM """
        # Save result of this API call into r_json variable
        r_json = apiapicem.ApiAPICEM.api_get_pnp_project()
        if not r_json == False:
            # Variable for indexing projects
    ...
    def pnp_APICEM_get_project_for_post_devices_toISE():
        """" Method for getting all projects for posting devices to APIC-EM and ISE """
        # Save result of this API call into r_json variable
        r_json = apiapicem.ApiAPICEM.api_get_pnp_project()
        if not r_json == False:
            # Variable for indexing projects
                ...
                try:
                    ...
                    if (cmd - 1 >= 0) and (cmd - 1 < len(r_json["response"])):
                        ...
                        if not os.path.isfile(PnP_APICEM.devices_tab):
                            ...
                            # Use the API call method with relevant parameters to create the devices in APIC-EM and ISE
